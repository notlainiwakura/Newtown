/**
 * Behavioral tests for the browser module, curiosity loop,
 * curiosity-offline loop, novelty engine, and book loop.
 *
 * These test the actual behavioral flows: browser launches, pages navigate,
 * content gets extracted, curiosity cycles run through all phases,
 * memories are saved with correct metadata, etc.
 */
...
let mockPage: Record<string, Mock>;
let mockContext: Record<string, Mock>;
let mockBrowser: Record<string, Mock>;

function createMockPage(overrides: Record<string, unknown> = {}) {
  return {
    goto: vi.fn().mockResolvedValue(undefined),
    title: vi.fn().mockResolvedValue('Test Page Title'),
    evaluate: vi.fn().mockResolvedValue('Extracted page content text here'),
    screenshot: vi.fn().mockResolvedValue(Buffer.from('fake-screenshot-bytes')),
...
function createMockContext(page?: Record<string, Mock>) {
  return {
    newPage: vi.fn().mockResolvedValue(page ?? mockPage),
    setDefaultTimeout: vi.fn(),
    close: vi.fn().mockResolvedValue(undefined),
  };
}

function createMockBrowser(ctx?: Record<string, Mock>) {
  return {
    newContext: vi.fn().mockResolvedValue(ctx ?? mockContext),
    close: vi.fn().mockResolvedValue(undefined),
  };
}
...
vi.mock('../src/security/ssrf.js', () => ({
  ...
  isAllowedDomain: vi.fn().mockReturnValue(true),
  isBlockedDomain: vi.fn().mockReturnValue(false),
}));

// ═══════════════════════════════════════════════════════════════
// 1. BROWSER BEHAVIORAL TESTS
// ═══════════════════════════════════════════════════════════════

describe('Browser behavioral', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  describe('browser launch returns usable instance', () => {
    it('launches browser and returns without error', async () => {
    ...
    it('browser becomes initialized after launch', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
    ...
    it('creates a browser context with user agent on launch', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser({ userAgent: 'TestBot/1.0' });
      expect(mockBrowser.newContext).toHaveBeenCalledWith(
        expect.objectContaining({ userAgent: 'TestBot/1.0' })
      );
      await closeBrowser();
    });

    it('sets timeout on context during initialization', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser({ timeout: 60000 });
      expect(mockContext.setDefaultTimeout).toHaveBeenCalledWith(60000);
      await closeBrowser();
    });

    it('uses default 30000ms timeout when none specified', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
  ...
  describe('navigate to URL and extract content', () => {
    it('navigates to the given URL via page.goto', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
    ...
    it('returns BrowseResult with url, title, content, and links', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('Clean text content')
        .mockResolvedValueOnce([{ text: 'Link 1', href: 'https://example.com/1' }]);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
    ...
    it('content extraction returns text without HTML tags (via evaluate script)', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      // The evaluate mock simulates what the script would return: clean text
      mockPage.evaluate
        .mockResolvedValueOnce('Just plain text, no <tags> here')
        .mockResolvedValueOnce([]);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://example.com');
      expect(result.content).toBe('Just plain text, no <tags> here');
      expect(result.content).not.toContain('<script');
      expect(result.content).not.toContain('<style');
      await closeBrowser();
    });

    it('uses domcontentloaded as default wait strategy', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
    ...
    it('can use networkidle wait strategy for JS-rendered pages', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await browse('https://spa-app.com', { waitFor: 'networkidle' });
      expect(mockPage.goto).toHaveBeenCalledWith(
        'https://spa-app.com',
    ...
    it('takes screenshot when requested and includes base64 in result', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://example.com', { screenshot: true });
      expect(mockPage.screenshot).toHaveBeenCalled();
      expect(result.screenshot).toMatch(/^data:image\/jpeg;base64,/);
      await closeBrowser();
    });

    it('does not include screenshot when not requested', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('content')
  ...
  describe('navigation timeout and error handling', () => {
    it('propagates navigation timeout error without crashing', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.goto.mockRejectedValue(new Error('Navigation timeout of 30000ms exceeded'));
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('https://slow-site.com')).rejects.toThrow('timeout');
      await closeBrowser();
    });
    ...
    it('closes the page even when navigation fails', async () => {
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await browse('https://nonexistent.invalid').catch(() => {});
      expect(mockPage.close).toHaveBeenCalled();
      await closeBrowser();
    });

    it('throws when browser is not initialized', async () => {
      const { browse } = await import('../src/browser/browser.js');
      await expect(browse('https://example.com')).rejects.toThrow('Browser not initialized');
    });

    it('handles evaluate failure gracefully (error propagated)', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate.mockRejectedValue(new Error('Execution context was destroyed'));
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('https://example.com')).rejects.toThrow();
      await closeBrowser();
    });
  });
  ...
  describe('content extraction edge cases', () => {
    it('returns empty string when page has no text content', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('')
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://empty-page.com');
      expect(result.content).toBe('');
      await closeBrowser();
    });

    it('truncates very large page content to 10000 chars', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://huge-page.com');
      expect(result.content.length).toBe(10000);
      await closeBrowser();
    });

    it('limits extracted links to max 50', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://link-heavy.com');
      // The actual script slices to 50, but our mock returns 60 — the test validates
      // the module does not crash and returns whatever evaluate produces
      expect(result.links).toHaveLength(60);
      await closeBrowser();
    });

    it('returns actual page URL (after redirects) via page.url()', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await browse('https://example.com/original');
  ...
  describe('SSRF protection before navigation', () => {
    it('blocks navigation to private IPs', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'Private IP address not allowed: 10.0.0.1' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('http://10.0.0.1')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('blocks navigation to localhost', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'Blocked hostname: localhost' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('http://localhost:3000')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('blocks navigation to 127.0.0.1', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'Private IP address not allowed: 127.0.0.1' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('http://127.0.0.1')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('blocks navigation to 192.168.x.x addresses', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'Private IP address not allowed: 192.168.1.1' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('http://192.168.1.1')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('blocks navigation to metadata endpoints (cloud provider)', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'Blocked hostname: 169.254.169.254' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(browse('http://169.254.169.254/latest/meta-data')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('allows navigation to public IPs when SSRF check passes', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true, resolvedIP: '93.184.216.34' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('content')
    ...
    it('SSRF check runs on screenshot function too', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'blocked' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, screenshot, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await expect(screenshot('http://10.0.0.1')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('SSRF check runs on evaluate function', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'blocked' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      await initBrowser();
      await expect(evaluate('http://10.0.0.1', 'document.title')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('SSRF check runs on fillForm function', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'blocked' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      await initBrowser();
      await expect(fillForm('http://10.0.0.1', { '#input': 'val' })).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });

    it('SSRF check runs on click function', async () => {
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: false, reason: 'blocked' });
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      await initBrowser();
      await expect(click('http://10.0.0.1', 'button')).rejects.toThrow('SSRF protection');
      await closeBrowser();
    });
  });

  // ── Browser close / resource cleanup ──────────────────────

  describe('browser close and resource cleanup', () => {
    it('closes context and browser on closeBrowser()', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
    ...
    it('marks browser as uninitialized after close', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
    ...
    it('closeBrowser is safe to call when not initialized', async () => {
      const { closeBrowser } = await import('../src/browser/browser.js');
      await expect(closeBrowser()).resolves.toBeUndefined();
    });

    it('closeBrowser handles context.close failure gracefully', async () => {
      const { chromium } = await import('playwright-core');
    ...
    it('closeBrowser handles browser.close failure gracefully', async () => {
      const { chromium } = await import('playwright-core');
  ...
  describe('multiple pages and simultaneous operations', () => {
    it('creates a new page for each browse call', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true, resolvedIP: '93.184.216.34' });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('content1').mockResolvedValueOnce([])
        .mockResolvedValueOnce('content2').mockResolvedValueOnce([]);
      const { initBrowser, browse, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await browse('https://page1.com');
      await browse('https://page2.com');
    ...
    it('screenshot function creates and closes its own page', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, screenshot, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
  ...
  describe('user agent string configuration', () => {
    it('sets default Chrome-like user agent', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
    ...
    it('allows overriding user agent string', async () => {
      const { chromium } = await import('playwright-core');
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser({ userAgent: 'CustomBot/2.0' });
      expect(mockBrowser.newContext).toHaveBeenCalledWith(
        expect.objectContaining({ userAgent: 'CustomBot/2.0' })
      );
      await closeBrowser();
    });
  });
  ...
  describe('form filling behavior', () => {
    it('fills form fields and returns updated page content', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      mockPage.evaluate
        .mockResolvedValueOnce('Form submitted!')
        .mockResolvedValueOnce([]);
      const { initBrowser, fillForm, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await fillForm('https://example.com/form', { '#name': 'Lain', '#email': 'lain@wired.jp' });
      expect(mockPage.fill).toHaveBeenCalledTimes(2);
      expect(mockPage.fill).toHaveBeenCalledWith('#name', 'Lain');
      expect(mockPage.fill).toHaveBeenCalledWith('#email', 'lain@wired.jp');
      expect(result.content).toBe('Form submitted!');
      await closeBrowser();
    });

    it('clicks submit button and waits for navigation when submitSelector provided', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, fillForm, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await fillForm('https://example.com', { '#q': 'test' }, 'button[type=submit]');
      expect(mockPage.click).toHaveBeenCalledWith('button[type=submit]');
      await closeBrowser();
    });

    it('does not click if no submitSelector provided', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, fillForm, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await fillForm('https://example.com', { '#q': 'test' });
  ...
  describe('click behavior', () => {
    it('navigates to page and clicks selector', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, click, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await click('https://example.com', '.btn');
      expect(mockPage.click).toHaveBeenCalledWith('.btn');
      expect(result.content).toBe('After click');
      await closeBrowser();
    });

    it('waits for navigation when waitForNavigation option is true', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      ...
      const { initBrowser, click, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      await click('https://example.com', '.link', { waitForNavigation: true });
  ...
  describe('screenshot function', () => {
    ...
    it('returns JPEG when type is jpeg', async () => {
      const { chromium } = await import('playwright-core');
      const { checkSSRF } = await import('../src/security/ssrf.js');
      (checkSSRF as Mock).mockResolvedValue({ safe: true });
      (chromium.launch as Mock).mockResolvedValue(mockBrowser);
      const { initBrowser, screenshot, closeBrowser } = await import('../src/browser/browser.js');
      await initBrowser();
      const result = await screenshot('https://example.com', { type: 'jpeg' });
...
// ═══════════════════════════════════════════════════════════════
// 2. CURIOSITY LOOP BEHAVIORAL TESTS
// ═══════════════════════════════════════════════════════════════
