...
describe('Sanitizer Fuzzing', () => {
  ...
  // findings.md P2:1222 — structural framing no longer escapes; input is preserved verbatim.
  it('structural framing preserves <script> verbatim (findings.md P2:1222)', async () => { const{sanitize}=await import('../src/security/sanitizer.js');const r=sanitize('<script>evil()</script>',{blockPatterns:false,warnPatterns:false,structuralFraming:true});expect(r.sanitized).toBe('<script>evil()</script>'); });
...
describe('SSRF Fuzzing', () => {
  ...
  // findings.md P2:1305 — sanitizeURL fuzz tests removed; the function
  // is no longer exported (safeFetch uses it internally).
});
...
describe('Config Fuzzing', () => {
  ...
  it('each missing required field → ValidationError', async () => { const{validate}=await import('../src/config/schema.js');const{ValidationError}=await import('../src/utils/errors.js');for(const k of['version','gateway','security','logging']){const p={...validConfig()} as Record<string,unknown>;delete p[k];expect(()=>validate(p),`missing ${k}`).toThrow(ValidationError);} });
  ...
  // findings.md P2:171 — agent-id / empty-agents / empty-providers / invalid-provider-type
  // cases moved to the character-manifest schema. See test/config-system.test.ts.
...
describe('Extra — validate() config edge cases', () => {
  ...
  // findings.md P2:171 — agent.enabled case covered by manifest schema, not LainConfig.
});
